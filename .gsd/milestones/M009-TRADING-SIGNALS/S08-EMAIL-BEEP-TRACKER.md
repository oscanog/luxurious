...
- [x] Create Convex action `convex/email.ts` with `"use node"` directive to send emails using `nodemailer` and Gmail app password from environment.
...
- No fallback app password is allowed in code or docs.
- **ACTION REQUIRED**: Set your actual Gmail address in Convex environment variables:
  `npx convex env set GMAIL_USER [GMAIL_USER_EMAIL]`
  `npx convex env set GMAIL_APP_PASSWORD [REDACTED_GMAIL_APP_PASSWORD]`
