...
import { User, Shield, Coins, Search, ArrowUpDown, ChevronRight, MoreVertical } from "lucide-react";
...
export function UserManager() {
  ...
  async function handleResetBalance(userId: any) {
    ...
    } catch (e) { toast.error("Failed to reset balance"); }
  }
              ...
              <tr key={u._id} className="hover:bg-[hsl(var(--muted)/0.1)] transition-colors group">
                ...
                <td className="px-6 py-4">
                  <div className={`inline-flex items-center gap-1.5 px-2 py-1 rounded-lg text-[10px] font-black uppercase tracking-wider ${
                    (u as any).isAdmin ? "bg-red-500/10 text-red-500" : "bg-blue-500/10 text-blue-500"
                  }`}>
                    <Shield size={10} />
                    {(u as any).isAdmin ? "Admin" : "Member"}
                  </div>
                </td>
                ...
                <td className="px-6 py-4 text-right">
                  <div className="flex items-center justify-end gap-2">
                    <button 
                      onClick={() => handleResetBalance(u._id)}
                    ...
                    <button 
                      onClick={() => handleToggleAdmin(u._id, (u as any).isAdmin)}
                      className={`p-2 rounded-lg bg-[hsl(var(--background))] border border-[hsl(var(--border))] transition-all ${
                        (u as any).isAdmin ? "text-red-500 hover:bg-red-500/5" : "text-[hsl(var(--muted-foreground))] hover:text-red-500"
                      }`}
                      title={(u as any).isAdmin ? "Revoke Admin" : "Make Admin"}
                    >
                      <Shield size={14} />
                    </button>
                  </div>
