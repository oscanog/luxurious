...
export type OrgCardData = {
  member: Omit<DummyMember, "id" | "uplineId" | "status" | "joinDate"> & {
    ...
    directStatusCounts?: Record<
      "joined" | "invited" | "pending" | "to-invite",
      number
    >;
    latestAsset?: {
      name: string;
...
export const OrgCardNode = memo(function OrgCardNode({
  data,
  selected,
}: NodeProps<OrgCardNodeType>) {
  ...
  const isFull = (member.directChildrenCount ?? 0) >= effectiveDirectLimit;
  ...
  const selectedStatusToneClass = {
    ...
    "to-invite": "scale-[1.05] z-10 shadow-[0_0_26px_rgba(148,163,184,0.18)]",
  } as const;
  ...
  const [removeMode, setRemoveMode] = useState<"disconnect" | "reconnect">(
    "disconnect",
  );
      ...
      if (previousUplineId) {
        void (async () => {
          try {
            await reassignMemberParent({
              memberId: member.id,
              newParentMemberId: previousUplineId,
            });
  ...
  const handleRightClick = useCallback(
    (e: React.MouseEvent) => {
      ...
      if (!isRoot && member.canManage !== false) {
        if (member.uplineId) {
          items.push({
    ...
    <>
      ...
      >
        ...
        <Handle
          ...
          className="!w-2 !h-2 !bg-[hsl(43,96%,48%)] !border-none opacity-0 group-hover:opacity-100 transition-opacity"
        />
        <Handle
          type="source"
        ...
        <div className="p-4 pb-6 group">
          ...
          <div className="flex items-start justify-between mb-4">
            <div className="flex items-center gap-3 overflow-hidden">
              ...
              <div className="flex flex-col min-w-0">
                <span className="text-[hsl(var(--foreground))] font-bold text-lg leading-tight truncate">
                  {member.name}
                </span>
                <span className="text-[hsl(var(--muted-foreground))] text-xs font-medium truncate">
                  {member.rank}
                </span>
                {member.latestAsset && (
                  <span className="text-[hsl(43,96%,48%)] text-[10px] font-black uppercase tracking-wider mt-0.5 truncate block">
                    Asset: {member.latestAsset.currency}{" "}
          ...
          <div className="flex items-center justify-between mb-4">
            <div
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full ${chipStyles[member.status].bg}`}
            >
            ...
            <span className="text-[hsl(var(--muted-foreground))] text-sm font-medium">
              {member.directChildrenCount || 0}/{effectiveDirectLimit}{" "}
          ...
          <div className="bg-[hsl(var(--muted)/0.5)] dark:bg-[#111827]/50 rounded-xl py-2.5 px-3 flex items-center justify-center gap-2">
            ...
            <span className="text-[hsl(var(--foreground))] text-sm font-medium">
              {member.totalDownlines || 0} total downline
            </span>
          </div>
        </div>
        ...
        <div className="space-y-3 mt-4">
          <label className="flex items-start gap-3 p-3 rounded-xl border border-[hsl(var(--border))] cursor-pointer hover:bg-[hsl(var(--muted)/0.5)] transition-colors">
            ...
            <div>
              ...
              <p className="text-xs text-[hsl(var(--muted-foreground))]">
                Member and their entire downline will be disconnected from the
                chart.
              </p>
            </div>
          </label>
          <label className="flex items-start gap-3 p-3 rounded-xl border border-[hsl(var(--border))] cursor-pointer hover:bg-[hsl(var(--muted)/0.5)] transition-colors">
            ...
            <div>
              ...
              <p className="text-xs text-[hsl(var(--muted-foreground))]">
                Member will be disconnected, but their direct downlines will be
                moved up to the grandparent.
              </p>
            </div>
          </label>
        </div>
